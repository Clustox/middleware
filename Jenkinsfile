pipeline {
    ...
    stages {
        ...
        // CLUSTOX: the SonarQube stages (analysis, quality gate, issue report)
        // were removed 2026-09-09 -- the Sonar server outage (502) was blocking
        // every deploy, and the team chose to drop the gate rather than keep an
        // analysis server on the critical path. Restore from git history if a
        // quality gate returns; sonar-project.properties went with it.
